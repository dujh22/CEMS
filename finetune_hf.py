...
def main(
        ...
        model_dir: Annotated[
            str,
            typer.Argument(
                help='A string that specifies the model id of a pretrained model configuration hosted on huggingface.co, or a path to a directory containing a model configuration file. 指定在huggingface.co上托管的预训练模型配置的模型id的字符串，或包含模型配置文件的目录的路径。'
            ),
        ], # 模型目录或ID
        config_file: Annotated[str, typer.Argument(help='')], # 配置文件
        auto_resume_from_checkpoint: str = typer.Argument(
            default='',
            help='If entered as yes, automatically use the latest save checkpoint. If it is a numerical example 12 15, use the corresponding save checkpoint. If the input is no, restart training 如果输入为yes，则自动使用最新的保存检查点。如果是数字示例12 15，则使用相应的保存检查点。如果输入为no，则重新开始训练。'
        ), # 自动从检查点恢复
